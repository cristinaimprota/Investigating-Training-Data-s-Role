...
def train():
    ...
    # Update the paths to use a list of files
    train_files = []

    raw_train_datasets = concatenate_datasets([
        load_dataset('json', data_files=file, split="train", cache_dir=training_args.cache_dir)
    ...
    for index in random.sample(range(len(eval_dataset)), 3):
        # print(f"Sample {index} of the evaluation set: {eval_dataset[index]['input_ids']}, {eval_dataset[index]['labels']}.")
        print(f"Sample {index} of the evaluation set: {tokenizer.decode(list(eval_dataset[index]['input_ids']))}.")
